...
class Themes_Asset_Db extends Database_Abstract {
	...
	public function alter_table() {
		$table = $this->get_table_name();
		$query = $this->wpdb->prepare( "ALTER TABLE {$table} ADD preset VARCHAR(20)" ); // phpcs:ignore

		return $this->wpdb->get_results( $query, ARRAY_A ); // phpcs:ignore
	}

	/* TODO : Remove this later! */
	public function alter_table_2() {
		$table = $this->get_table_name();
		$query = $this->wpdb->prepare( "ALTER TABLE {$table} ADD enqueue VARCHAR(20)" ); // phpcs:ignore
